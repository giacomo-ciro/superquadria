...
def check(seed, overrides=None):
    ...
    for door in w.doors:
        ...
        if abs(prim.scale[1] * 2 - tall) > 1e-6:
            problems.append(
                f"door {door['id']} height {prim.scale[1] * 2:.2f} != {tall}"
            )
    ...
    for door in w.doors:
        ...
        for axis in IN_PLANE[door["axis"]]:
            if lo[axis] < blo[axis] - 1e-6 or hi[axis] > bhi[axis] + 1e-6:
                problems.append(
                    f"door {door['id']} escapes its room's shell on axis {axis}"
                )
                break
    ...
    if leaks:
        problems.append(
            f"shell leaks: the player flies out through "
...
if __name__ == "__main__":
    ...
    for seed, over in cases:
        ...
        print(
            f"{'FAIL' if problems else 'ok  '} {tag:<58} "
