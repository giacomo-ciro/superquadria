...
def _generate(config: DictConfig, *, offline: bool) -> Scene:
    ...
    print(f"[gen] {len(scene.primitives)} primitives, {len(scene.meta['layout']['rooms'])} room(s) "
...
def _dispatch(argv: list[str] | None = None) -> int:
    ...
    if args.command == "play":
        ...
        print(f"[play] {len(scene.primitives)} primitives, spawn {scene.player.position}")
