"""`TmuxAgent` — the shared machinery for driving an interactive CLI in tmux.
...
Subclasses supply what differs between CLIs: the command that spawns the interactive
process (`_spawn_command`) and the pane markers that mean "a turn is running"
(`busy_regex`).
"""
...
class TmuxAgent(Agent):
    ...
    def cancel(self) -> None:
        ...
        if self._is_initialized:
            subprocess.run(["tmux", "send-keys", "-t", self.target, "Escape"], check=False)

    # ----------------------------------------------------------------- private

    def _session_exists(self) -> bool:
        return subprocess.run(["tmux", "has-session", "-t", f"={self.session_name}"],
    ...
    def _ensure_session(self) -> None:
        ...
        # Tear down the leftover window *before* checking that the session exists, not
        # after. Killing a session's last window takes the session with it, so a
        # previous run whose window was the only one left (its shell window closed, or
        # the run only ever used one role) had the session vanish underneath it and the
        # `new-window` below failed outright — the restart burned a retry on cleanup
        # that had in fact worked. check=False because the leftover CLI may also have
        # exited on its own between the listing and the kill, which closes the window.
        if self._session_exists():
            res = subprocess.run(
                ["tmux", "list-windows", "-t", f"={self.session_name}", "-F", "#{window_name}"],
                capture_output=True, text=True, check=True
            )
            if self.window_name in res.stdout.strip().split("\n"):
                subprocess.run(["tmux", "kill-window", "-t", self.target], check=False)

        if not self._session_exists():
            subprocess.run(["tmux", "new-session", "-d", "-s", self.session_name,
                            "-x", str(SESSION_WIDTH), "-y", str(SESSION_HEIGHT)], check=True)

        # Replicates fm_backend_tmux_create_task logic.
        # No `-d`: the new window becomes the session's current one, so a client that
        # attaches to watch lands on the CLI instead of the session's shell window —
        # including after a retry tears this window down and builds another.
        cmd = ["tmux", "new-window", "-P", "-F", "#{window_id}", "-t", f"={self.session_name}:",
               "-n", self.window_name, "-c", self.workdir]
        # We spawn the interactive CLI process directly
        ...
        subprocess.run(["tmux", "set-window-option", "-t", wid, "automatic-rename", "off"], check=True)
        subprocess.run(["tmux", "set-window-option", "-t", wid, "allow-rename", "off"], check=True)

        # Pin the window size. `new-session -x/-y` only covers a server with no clients
        # at all: the default `window-size latest` resizes the window to the terminal of
        # whichever client was used last, even one attached to an unrelated session, so
        # in practice the pane silently took the size of whatever else was open (200x50
        # asked for, 178x19 observed). Since both CLIs hard-wrap to the pane width with
        # real newlines, that quietly mangles long JSON fields. `resize-window` sets
        # this window to `window-size manual`, which no attaching client can override.
        subprocess.run(["tmux", "resize-window", "-t", wid,
                        "-x", str(SESSION_WIDTH), "-y", str(SESSION_HEIGHT)], check=True)
        ...
        self.log.info(self.stats["calls"] + 1,
                      f"watch this agent: tmux attach -f ignore-size -t {self.session_name}")
    ...
    def _capture(self) -> str:
        res = subprocess.run(["tmux", "capture-pane", "-p", "-t", self.target, "-S", "-40"],
                             capture_output=True, text=True, check=True)
        return res.stdout
    ...
    def _iter_balanced_objects(text: str):
        """Yield each top-level `{...}` span in `text`, ignoring braces inside strings."""
        depth = 0
        in_string = False
        escape = False
        start = None
    ...
    def _invoke(self, prompt: str, schema: dict, *, system: str | None) -> dict:
        ...
        full_prompt += "\nWrap your output in a fenced code block labeled json, and output nothing else."
        ...
        subprocess.run(["tmux", "set-buffer", full_prompt], check=True)
        subprocess.run(["tmux", "paste-buffer", "-p", "-t", self.target], check=True)
