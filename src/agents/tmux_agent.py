...
class TmuxAgent(Agent):
    ...
    session_name: str = "general-intuition-3d"
    ...
    def clean_session_windows(cls, session_name: str) -> None:
        ...
        if not subprocess.run(["tmux", "has-session", "-t", f"={session_name}"],
        ...
        standby_res = subprocess.run(
            ["tmux", "new-window", "-P", "-F", "#{window_id}", "-t", f"={session_name}:",
             "-n", "standby", "tail", "-f", "/dev/null"],
            capture_output=True, text=True
        )
        ...
        with _SESSION_LOCK:
            ...
            if not self._session_exists():
                cmd = ["tmux", "new-session", "-d", "-s", self.session_name,
                       ...
                       "-x", str(SESSION_WIDTH), "-y", str(SESSION_HEIGHT),
                       "-P", "-F", "#{window_id}"]
                ...
                res = subprocess.run(
                    ["tmux", "list-windows", "-t", f"={self.session_name}", "-F", "#{window_id}:#{window_name}"],
                ...
                # Create the new window first to ensure the session never reaches 0 windows
    ...
    def _invoke(self, prompt: str, schema: dict, *, system: str | None) -> dict:
        ...
        subprocess.run(["tmux", "set-buffer", "-b", buf_name, "--", full_prompt], check=True)
        subprocess.run(["tmux", "paste-buffer", "-p", "-t", self.target, "-b", buf_name], check=True)
