...
def nav_user_prompt(state: State, memory: SpatialMemory, *,
                    sensor_range: float, max_waypoints: int, max_segment: float,
                    call_budget: int | None = None, distance_budget: float | None = None,
                    collision_budget: int | None = None) -> str:
    """Prompt asking the navigation agent to plan a batch of actions from its current observation."""
    ...
    if state.carrying is not None:
        carrying_line = (f"You are CARRYING: {state.carrying['assembly']} ({state.carrying['parts']} part(s)). "
                         f"Place it at this room's locked door to attempt "
                         f"opening it, or drop it on the floor if you want to free your hands.")
    else:
        carrying_line = ("You are carrying nothing. Fly close to any assembly (<= 2.5 units) "
    ...
    return f"""Find the object matching the required shape name, carry it to the door, and open it. Each
room on your route has its own locked door requiring its own key shape — solving this one only lets you
into the next room, which has a new lock requirement.

The building is made entirely of superquadrics. Walls, floors, ceilings, furniture and the
locked door are solid and block you.

You are in {state.room or "an unnamed room"}, at {state.player_position}, looking along
...
- Explore the room and inspect the assemblies detected by your sensor. Each assembly (assembly-0, assembly-1, ...)
...
READING A SHAPE: `assembly` groups primitives that belong together as parts of one object (assembly-0, assembly-1, ...).
Walls, floor, ceiling, and the door (assembly 'door') form the room structure. `centre` is its middle in world coordinates,
`dist` is Euclidean distance from you, `size` is its half-extent along its own three axes before rotation, and `exps` set its form —
(1, 1) is an ellipsoid, (0.1, 0.1) a box, (0.1, 1) a cylinder, above 2 a pinched, concave shape.
